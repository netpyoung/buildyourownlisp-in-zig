...
pub fn build(b: *std.Build) void {
    ...
    const root_module = b.createModule(.{
        .root_source_file = b.path("src/ch14.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    { // c string
        const translate_string = b.addTranslateC(.{
            .root_source_file = b.path("src/c_string.h"),
            .target = target,
            .optimize = optimize,
            .link_libc = true,
        });

        const string_module = translate_string.createModule();
        root_module.addImport("c_string", string_module);
    }
